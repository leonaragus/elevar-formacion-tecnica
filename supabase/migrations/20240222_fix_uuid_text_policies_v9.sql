-- SOLUCIÓN FINAL V9: Detección Automática de FK y Conversión Universal
-- Este script es "nuclear": detecta CUALQUIER tabla que apunte a cursos.id y la convierte a TEXTO
...
DECLARE
    r RECORD;
BEGIN
    -- A. Buscar TODAS las FK que apuntan a cursos.id
    FOR r IN (
        SELECT 
            tc.table_schema, 
            tc.table_name, 
        ...
        -- Convertir columna a TEXT (si existe y no lo es ya)
        ...
        -- Limpiar huérfanos (temporalmente convertimos id de cursos a text para comparar)
        EXECUTE format('DELETE FROM %I.%I WHERE %I::text NOT IN (SELECT id::text FROM cursos)', r.table_schema, r.table_name, r.column_name);
    END LOOP;
END $$;

-- 3. CAMBIO CRÍTICO: Convertir cursos.id a TEXTO (ahora que no hay FKs apuntando)
ALTER TABLE cursos ALTER COLUMN id TYPE TEXT USING id::text;

-- 4. Restaurar FKs (Detectadas manualmente las importantes)
DO $$ 
BEGIN
    -- Mensajes
    ...
    -- Materiales (la que causó el error)
