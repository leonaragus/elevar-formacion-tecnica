-- SOLUCIÓN FINAL V7: Corregir tipos con CASTING EXPLÍCITO (A PRUEBA DE FALLOS)
-- Este script soluciona el error "operator does not exist: text = uuid" casteando todo a texto.
...
-- 2. Corrección SEGURA de tipos (usando ::text en comparaciones)
DO $$ 
BEGIN
    -- --- CORRECCIÓN TABLA MENSAJES ---
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'mensajes') THEN
        -- Quitar constraint para poder modificar libremente
        ALTER TABLE mensajes DROP CONSTRAINT IF EXISTS mensajes_curso_id_fkey;
        
        -- Limpiar huérfanos usando CASTING EXPLÍCITO para evitar error uuid=text
        -- Comparamos texto con texto siempre
        DELETE FROM mensajes WHERE curso_id::text NOT IN (SELECT id::text FROM cursos);
        
        -- Ahora sí, convertir a TEXT si es necesario
        -- Si ya es texto, esto no hace daño. Si es uuid, lo convierte.
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'clases_grabadas') THEN
        ...
        -- Limpiar huérfanos con CASTING EXPLÍCITO
...
-- 3. Recrear Políticas RLS (SEGURAS DE TIPOS y SIN USUARIOS EXTERNOS)
...
CREATE POLICY "Alumnos pueden ver clases de sus cursos" ON clases_grabadas
  ...
  USING (
    EXISTS (
      ...
      WHERE cursos_alumnos.curso_id = clases_grabadas.curso_id -- Ambos son TEXT
