...
export default function FloatingFeedbackButton({ cursoId, claseId }: { cursoId: string, claseId: string }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div className="absolute bottom-20 right-0 w-full max-w-md bg-gray-800/90 backdrop-blur-md rounded-xl shadow-2xl border border-gray-700/50 overflow-hidden animate-fade-in-up">
          ...
          <div className="max-h-[60vh] overflow-y-auto p-4 bg-gray-900/50">
            {activeTab === 'comments' ? (
              // El nuevo componente de comentarios, con el input y la lista moderna
              <CommentsCourseSummary cursoId={cursoId} claseId={claseId} />
            ) : (
              <div className="flex flex-col items-center justify-center h-full space-y-4 py-8">
                <h3 className="text-lg font-semibold text-white">¿Qué te pareció la clase?</h3>
