// Script para ejecutar las migraciones de clases grabadas directamente
      ...
      for (let i = 0; i < statements.length; i++) {
        ...
        if (statement) {
          try {
            ...
            if (stmtError) {
              console.warn(`⚠️ Error en statement ${i + 1}:`, stmtError);
            } else {
              console.log(`✅ Statement ${i + 1} ejecutado`);
            }
          } catch (err) {
            console.warn(`⚠️ Excepción en statement ${i + 1}:`, err);
          }
        }
      }
    } else {
